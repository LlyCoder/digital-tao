<template lang="html">
  <div>
    ...
    <div class="checkout-page">
      ...
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkedIndex == index}" @click="checkedIndex=index;selectedAdsId=item.addressId">
                <dl>
                  <dt>{{item.userName}}</dt>
              ...
              <li class="addr-new" @click="inputModal=true">
            ...
            <ul>
              <li class="check">
                ...
                <div class="shipping-tips">
                  <p>快递公司由卖家选择，卖家承诺在七天之后到达你的所在地，不可中途取消。本站不作任何担保。</p>
                </div>
              </li>
            </ul>
          </div>
        ...
        <div class="next-btn-wrap">
          <router-link class="btn btn--m btn--red" v-bind:to="{path:'orderConfirm', query:{'addressId':selectedAdsId}}">Next</router-link>
        </div>
      </div>
    </div>
    </div>
    ...
    <modal :mdShow="inputModal" @close="closeInputModal">
      <p slot="message">
        <el-input placeholder="" v-model="userName" @blur="checkInput('reName')">
          <template slot="prepend">收货人</template>
        </el-input>
        <span class="err-tip" v-show="reNameErr">收货人必填</span>
        <el-input placeholder="" v-model="streetName"  @blur="checkInput('adName')">
          <template slot="prepend">街道名</template>
        </el-input>
        <span class="err-tip" v-show="adNameErr">街道名必须填写</span>
        <el-input placeholder="" v-model="postCode"  @blur="checkInput('postCode')">
          <template slot="prepend">邮编号</template>
        </el-input>
        <span class="err-tip" v-show="postCodeErr">邮编号须为数字</span>
        <el-input placeholder="" v-model="tel"  @blur="checkInput('tel')">
          <template slot="prepend">手机号</template>
        </el-input>
        <span class="err-tip" v-show="telErr">手机号格式不正确</span>
      </p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="inputModal=false">取消</a>
        <a class="btn btn--m" href="javascript:;" @click="addAddress">新增</a>
      </div>
    </modal>
...
export default {
  ...
  methods: {
    ...
    checkInput(choice) {
      switch (choice) {
        ...
        case 'adName': 
          ...
          break;
        case 'postCode': 
          //不能用isNaN来判断，巨坑，空字符会被隐式转换成0，导致返回false
          //利用正则
          const reg = /^[0-9]+.[0-9]*$/; 
          if (reg.test(this.postCode)) {
            this.postCodeErr = false;
          } else {
            this.postCodeErr = true;
          }
        case 'tel': 
          const pattTel = /^[1][3,4,5,7,8][0-9]{9}$/;
          if (pattTel.test(this.tel)) {
            this.telErr = false;
          } else {
            this.telErr =true;
          }
    ...
    setDefault(addressId) {
      axios.post('/users/setAdsDefault', {
        addressId: addressId
      }).then((response) => {
        let res = response.data;
        if(res.status == '0') {
    ...
    addAddress() {
      if (this.reNameErr ==false && this.adNameErr == false && this.postCodeErr == false && this.telErr == false ) {
        if(this.userName.trim() && this.postCode.trim() && this.streetName.trim() && this.tel.trim()) {        
